...
export default function EmployeePage() {
  ...
  const doAction = async (path: string, actionLabel: string) => {
    ...
    try {
      ...
      if (!qrPayload.trim()) throw new Error("Paste QR payload first");

      const result = await apiFetch<any>(path, {
        method: "POST",
        body: JSON.stringify({ officeId, qrPayload, gps }),
      });
              ...
              <div>
                ...
                {qrPayload ? (
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-xs text-emerald-600 font-medium">✓ QR payload ready</span>
                    <button
                      type="button"
